...
if (!@file_exists($request_text)) {
    header( "HTTP/1.1 404 Not Found" );
    exit;
}
if (@is_dir($request_text)) {
  header( "HTTP/1.1 404 Not Found" );
  exit;
}
class ResumeDownload {
    ...
    function __construct($file,$filename="",$delay = 0) {
        if (! is_file($file)) {
...
set_time_limit(0);
$download = new ResumeDownload($request_text,$_GET['q']);
$download->process();
